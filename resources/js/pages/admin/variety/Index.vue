...
const storage = usePageStorage('verieties')
...
const deleteItem = (row) => handleDelete({
  ...
  url: route("admin.variety.delete", row.id),
...
const fetchItems = (props = null) => handleFetchItems({
  ...
  props,
  rows,
  url: route("admin.variety.data"),
  loading,
});

const onFilterChange = () => fetchItems();
const onRowClicked = (row) => router.get(route('admin.variety.detail', { id: row.id }));
...
<template>
  ...
  <authenticated-layout>
    ...
    <template #right-button>
      <q-btn icon="add" dense color="primary" @click="router.get(route('admin.variety.add'))" />
      ...
      <q-btn icon="file_export" dense class="q-ml-sm" color="grey" style="" @click.stop>
        <q-menu anchor="bottom right" self="top right" transition-show="scale" transition-hide="scale">
          <q-list style="width: 200px">
            <q-item clickable v-ripple v-close-popup :href="route('admin.variety.export', { format: 'pdf' })">
              <q-item-section avatar>
                <q-icon name="picture_as_pdf" color="red-9" />
              </q-item-section>
              <q-item-section>Export PDF</q-item-section>
            </q-item>
            <q-item clickable v-ripple v-close-popup :href="route('admin.variety.export', { format: 'excel' })">
              <q-item-section avatar>
                  ...
                  <q-menu anchor="bottom right" self="top right" transition-show="scale" transition-hide="scale">
                    <q-list style="width: 200px">
                      <q-item clickable v-ripple v-close-popup
                        @click.stop="router.get(route('admin.variety.duplicate', props.row.id))">
                        <q-item-section avatar>
                      ...
                      <q-item clickable v-ripple v-close-popup
                        @click.stop="router.get(route('admin.variety.edit', props.row.id))">
                        <q-item-section avatar>
                          <q-icon name="edit" />
                        </q-item-section>
